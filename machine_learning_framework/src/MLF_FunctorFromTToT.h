...
class SigmoidFunctorFromTToT :public FunctorFromTToT<T>
{
	const T a;
	const T b;
	static_assert(std::is_floating_point<T>::value, "This sigmoid implementation has no sense working with non-floating point types"); //technically a HUUGE (like int's max value) c value might make it useful, but just use your own kind of functor
public:
...
class AtanFunctorFromTToT :public FunctorFromTToT<T>
{
	const T a;
	const T b;
	static_assert(std::is_floating_point<T>::value, "This arctan implementation has no sense working with non-floating point types"); //technically a HUUGE (like int's max value) c value might make it useful, but just use your own kind of functor
public:
